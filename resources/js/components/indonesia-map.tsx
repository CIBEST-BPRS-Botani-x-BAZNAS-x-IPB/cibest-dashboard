...
export function IndonesiaMap({
        ...
        script.onload = () => {
            ...
            fetch(geojsonPath)
                .then((res) => {
                    ...
                    if (!res.ok) {
                        throw new Error(
                            `HTTP ${res.status}: Failed to load GeoJSON from ${res.url}`,
                        );
                    }
                    return res.json();
                })
                    ...
                    const geoJsonLayer = L.geoJSON(data, {
                        style: (feature: any) => {
                            ...
                            for (const prov of provinces) {
                                ...
                                if (
                                    geoProvinceName.includes(
                                        compareProvinceName,
                                    ) ||
                                    compareProvinceName.includes(
                                        geoProvinceName,
                                    )
                                ) {
                                    matchingProvince = prov;
    ...
    }, [view]); // Add view to dependencies to re-render map on toggle
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                    <button
                        ...
                        style={{
                            ...
                            backgroundColor: view === 'before' ? '#14b8a6' : '#f1f5f9',
                            color: view === 'before' ? 'white' : '#64748b',
                        }}
                        onMouseEnter={(e) => {
                            if (view === 'before') {
                    ...
                    <button
                        ...
                        style={{
                            ...
                            transition: 'all 0.2s',
                            boxShadow: '0 1px 2px 0 rgb(0 0 0 / 0.05)',
                            backgroundColor: view === 'after' ? '#14b8a6' : '#f1f5f9',
                            color: view === 'after' ? 'white' : '#64748b',
                        }}
                        onMouseEnter={(e) => {
                            if (view === 'after') {
                                e.currentTarget.style.backgroundColor = '#0d9488';
